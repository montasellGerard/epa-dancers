...
const STATS = [
  { val: 8,    suffix: '+', lbl: 'Años'       },
  { val: 4,    suffix: '',  lbl: 'Niveles'     },
  { val: 15,   suffix: '+', lbl: 'Clases/sem'  },
  { val: 80,   suffix: '+', lbl: 'Alumnos'     },
]
...
function StatItem({ val, suffix, lbl, active }: { val: number; suffix: string; lbl: string; active: boolean }) {
  ...
  return (
    <div>
      <span className="block font-black text-2xl" style={{ color: '#F0B429' }}>
        {count}{suffix}
      </span>
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 w-full flex flex-col md:flex-row items-center gap-12 py-32 md:py-0">
        ...
        <div className="flex-1 flex flex-col justify-center">
          {/* Eyebrow */}
          <div
            className="inline-flex items-center gap-2 w-fit mb-6 px-4 py-2 rounded-full text-[10px] tracking-[3px] uppercase font-bold font-sans"
            style={{
          ...
          <p
            className="text-white/60 font-sans text-base leading-relaxed mb-8 max-w-md"
            style={{
              animation: 'fadeInUp 0.6s ease forwards',
          ...
          >
            <a
              ...
              style={{
                ...
                boxShadow: '0 6px 24px rgba(224,21,122,0.4)',
              }}
            >
              ¡Quiero empezar!
            </a>
          ...
          >
            {STATS.map((s) => (
              <StatItem key={s.lbl} val={s.val} suffix={s.suffix} lbl={s.lbl} active={statsVisible} />
            ))}
          </div>
        </div>

        {/* Photo — placeholder hasta disponer de foto real */}
        <div className="w-full md:w-[340px] flex-shrink-0 flex items-end justify-center">
          <div
            className="relative w-full max-w-sm aspect-[3/4] rounded-2xl overflow-hidden flex flex-col items-center justify-center gap-4"
            style={{
              border: '2px solid rgba(255,255,255,0.08)',
              boxShadow: '0 0 60px rgba(0,201,177,0.12), 0 0 100px rgba(224,21,122,0.08)',
              background: 'linear-gradient(160deg, rgba(0,201,177,0.07) 0%, rgba(14,11,6,1) 50%, rgba(224,21,122,0.07) 100%)',
            }}
          >
            {/* Icono decorativo */}
            <div
      ...
      {/* Scroll indicator */}
      <div
        className="absolute left-1/2 bottom-8 flex flex-col items-center gap-1.5 pointer-events-none"
        style={{ animation: 'bounce-y 2s ease-in-out infinite', transform: 'translateX(-50%)' }}
        aria-hidden="true"
      >
        <span className="font-sans text-[9px] uppercase tracking-[2px]" style={{ color: 'rgba(240,180,41,0.5)' }}>
          Scroll
        </span>
