...
// ─── Filter pills ─────────────────────────────────────────────────────────────
const FILTERS: { label: string; value: GalleryCategory | 'todos' }[] = [
  { label: 'Todos',      value: 'todos'     },
  { label: 'Clases',     value: 'clase'     },
  { label: 'Congresos',  value: 'congreso'  },
  { label: 'Actuaciones',value: 'actuacion' },
...
function GalleryCard({ item }: { item: GalleryItem }) {
  ...
  return (
    ...
    >
      ...
      <span
        className="absolute top-3 left-3 font-sans font-extrabold text-[9px] tracking-[2px] uppercase px-2.5 py-1 rounded-full"
        style={{
      ...
      >
        <p className="font-sans font-semibold text-[11px] leading-snug text-white/90">
          {item.title}
        </p>
        <p className="font-sans text-[9px] text-white/40 mt-0.5 uppercase tracking-widest">
          Próximamente
        </p>
      </div>
    </article>
...
export default function Gallery() {
  ...
  // Reset scroll + index on filter change
  useEffect(() => {
    setActiveIndex(0)
    scrollRef.current?.scrollTo({ left: 0, behavior: 'smooth' })
  }, [activeFilter])

  // Scroll tracker — dots + arrow state
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="px-6 max-w-7xl mx-auto mb-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <h2
              ...
              style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(28px,4vw,40px)', color: '#fff' }}
            >
              El baile en{' '}
              <span
                style={{
                  background:            'linear-gradient(90deg, #F0B429, #F45E0C)',
            ...
            {/* Desktop arrows */}
            <div className="hidden sm:flex items-center gap-2" aria-label="Navegación de galería">
              <button
          ...
          {/* Filter pills */}
          <div className="flex flex-wrap gap-2 mt-6" role="group" aria-label="Filtrar por categoría">
            {FILTERS.map((f) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: isActive ? '#F0B429'                : 'rgba(255,255,255,0.07)',
                    color:      isActive ? '#0E0B06'               : 'rgba(255,255,255,0.5)',
        ...
        <div
          ref={scrollRef}
          className="scroll-hidden flex gap-4 overflow-x-auto snap-x snap-mandatory pb-4"
        ...
        >
          ...
          {/* Ghost card — trailing space so last card snaps cleanly */}
            ...
            {filtered.map((item, i) => (
              <span
                ...
                style={{
                  ...
                  height:     '6px',
                  background: activeIndex === i ? '#F0B429' : 'rgba(255,255,255,0.2)',
                }}
              />
            ))}
          </div>
        ...
        {/* Bottom note */}
        <p
          className="text-center font-sans text-[11px] mt-6 px-6"
          style={{ color: 'rgba(255,255,255,0.2)' }}
        >
          Próximamente · Clases, congresos y actuaciones de Alicia y Pedro
        </p>
      </div>
    </section>
  )
